...
# 设置工作目录
setwd("/home/vensin/Rstudio/RStudio/Population_and_Landscape_Genomics_of_Sweet_Osmanthus/GEA_2025")

# 读取坐标文件
coordinates <- read.csv("202samples_lon_lat_ele.csv")
# 确保经纬度列为数值型
...
for (var_name in names(fixed_env_files)) {
  f_path <- file.path(tif_directory, fixed_env_files[[var_name]])
  if (file.exists(f_path)) {
    r <- rast(f_path)
    # terra::extract 返回两列: ID 和 Value，我们取第二列
    base_data[[var_name]] <- extract(r, coords_mat)[, 2]
  } else {
    warning(paste("固定变量文件未找到:", f_path))
    base_data[[var_name]] <- NA
  }
}
...
for (combo in scenario_time_combinations) {
  ...
  if (all(file.exists(bio_files))) {
    ...
    # 3. 调整列顺序 ------------------------------------------------------------
    # 确保列名存在再排序，防止出错
    target_cols <- c("ind", "lon", "lat", paste0("BIO", 1:19), "Elevation", "SRAD", "SOC", "PHH2O")
    # 只选择存在的列
    ...
    output_file_name <- sprintf("Climate_%s_%dsamples.csv", combo, nrow(final_result))
