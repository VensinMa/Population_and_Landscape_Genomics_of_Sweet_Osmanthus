...
# 设置工作目录 (请确保路径正确)
setwd("/home/vensin/Rstudio/RStudio/Population_and_Landscape_Genomics_of_Sweet_Osmanthus/GEA_2025/lfmm/indel")
cat("当前工作目录:", getwd(), "\n")
...
if (ncol(Y) != nrow(indel_ids)) stop("错误：Y 矩阵的列数与 indel ID 文件的行数不匹配！")
...
# 准备数据框
# 注意：data.table::fwrite 对 row.names=TRUE 支持一般，建议显式创建 INDEL_ID 列
df_raw <- data.frame(INDEL_ID = indel_ids$V1, raw_pvalues)
df_cal <- data.frame(INDEL_ID = indel_ids$V1, calibrated_pvalues)

# 保存合并后的数据
fwrite(df_raw, file = "indel_raw_pvalues_merged.csv", sep = ",")
fwrite(df_cal, file = "indel_calibrated_pvalues_merged.csv", sep = ",")
...
# 合并 INDEL ID 和 Q 值
df_final_q <- data.frame(INDEL_ID = indel_ids$V1, all_qvalues)
...
all_sig_indels_vector <- c() # 用于存储所有显著位点的向量 (用于求并集)
...
for (i in 1:num_env_factors) {
  ...
  # --- 3. 保存该环境因子的所有结果 ---
  # 包含 INDEL ID, P值, Q值
  res_df <- data.frame(INDELid = indel_ids$V1, pvalues = pvals, qvalues = qvals)
  ...
  if (length(sig_idx_05) > 0) {
    ...
    # 提取信息
    sig_indels <- indel_ids$V1[sig_idx_05]
    sig_p <- pvals[sig_idx_05]
    sig_q <- qvals[sig_idx_05]
    
    # 保存显著结果详情
    sig_df <- data.frame(INDELid = sig_indels, pvalues = sig_p, qvalues = sig_q)
    fwrite(sig_df, file = paste0("significant_results_", curr_env, ".csv"), row.names = FALSE)
    
    # 仅保存 INDEL ID 列表 (用于后续提取基因等)
    fwrite(data.frame(INDELid = sig_indels), file = paste0("significant_indel_ids_", curr_env, ".csv"), row.names = FALSE, col.names = FALSE)
    
    # 添加到总向量中 (用于后续求并集)
    all_sig_indels_vector <- c(all_sig_indels_vector, sig_indels)
    
    # 记录数量
    count_val <- length(sig_indels)
    
  } else {
    count_val <- 0
  }
  
  significant_counts_list[[i]] <- data.frame(Environment_Factor = curr_env, Significant_INDEL_Count = count_val)
}
...
fwrite(counts_df, file = "Significant_INDEL_Counts.csv")
...
# 去除重复，得到唯一的显著位点列表
union_sig_indels <- unique(all_sig_indels_vector)
union_count <- length(union_sig_indels)

# 保存并集结果
if (union_count > 0) {
  union_df <- data.frame(INDELid = union_sig_indels)
  fwrite(union_df, file = "LFMM_significant_indel_ids_union.csv", row.names = FALSE)
