...
# 1. 设置工作目录与数据加载
# ==============================================================================
setwd("/home/vensin/Rstudio/RStudio/Population_and_Landscape_Genomics_of_Sweet_Osmanthus/GEA_2025/RDA/snp")

# 如果目录不存在，自动创建
if(!dir.exists(getwd())) dir.create(getwd(), recursive = TRUE)

# --- 加载基因型数据 ---
rawgeno <- fread("/home/vensin/workspace/snpcalling_wild/11.vcftools_filter/snp/202_samples_snp_filtered.missing0.05.plink.recodeA.lfmm")
dim(rawgeno) ## [1] 202 10590591

# 读取 SNP 的 ID 文件
snp_ids <- fread("/home/vensin/workspace/snpcalling_wild/11.vcftools_filter/snp/202_samples_snp_filtered.missing0.05.ID", 
                 header = FALSE)
dim(snp_ids) ## [1] 10590591  1

# 检查维度
if (ncol(rawgeno) != nrow(snp_ids)) stop("SNP 标识符的数量与基因型数据的列数不一致。")
...
# 保存插补后的基因型数据
save(gen.imp, file = "gen.imp_SNP.RData") 
load("gen.imp_SNP.RData")
...
save(of.rda, file = "of.rda_SNP.RData") 
...
# 保存特征值表
write.csv(eig_summary, file = "Importance_of_components_eigenvals_RDA_SNP.csv")
...
print(paste("共检测到候选 SNP 数量:", length(combined_cand)))
## 共检测到候选 SNP 数量: 338502
...
# 保存候选位点
write.csv(data.frame(names = combined_cand), "RDA_cand_IDs_SNP.csv", row.names = FALSE)

# 保存候选位点详细信息
write.csv(cand_df_full, "RDA_cand_Details_SNP.csv", row.names = FALSE)

# 检查是否有 SNP 同时在多个轴上显著
duplicate_snps <- cand_df_full$snp[duplicated(cand_df_full$snp)]
if(length(duplicate_snps) > 0) {
  print(paste("有", length(unique(duplicate_snps)), "个 SNP 在多个轴上均被识别为离群点"))
}
## 有 31023 个 SNP 在多个轴上均被识别为离群点
